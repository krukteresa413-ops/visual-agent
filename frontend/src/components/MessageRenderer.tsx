...
export default function MessageRenderer({ message, isLight = true }: Props) {
  ...
  const isDone = message.status === 'completed';
  ...
  return (
    <div data-message-renderer className={`flex text-xs ${isUser ? 'justify-end' : 'justify-start'}`}>
      <div
        className={`max-w-[86%] rounded-2xl border px-3 py-2 shadow-sm ${isError ? 'border-red-200 bg-red-50' : ''} ${isDone ? 'border-green-200 bg-green-50' : ''}`}
        style={isError || isDone ? undefined : shellStyle}
      >
        ...
        {message.assets.length > 0 && (
          <div className="mt-2 grid gap-2">
            {message.assets.map((asset) => {
              if (asset.type === 'image') {
                return <img data-message-image key={asset.url} src={asset.url} alt={message.step || 'MOYAG image result'} className="max-h-44 w-full rounded-lg object-cover" />;
              }
              if (asset.type === 'video') {
