...
class TestHistoryAPI:
    ...
    def test_list_history_returns_records(self, mock_session_cls):
        ...
        mock_record.asset_plan_json = json.dumps({"main_image": {"goal": "test"}})
...
class TestAutoSave:
    ...
    def test_generate_all_parallel_accepts_db_param(self):
        """generate_all_parallel 接受 db 参数。"""
        import inspect
        from app.services.visual_agent import VisualAgent
        sig = inspect.signature(VisualAgent.generate_all_parallel)
        assert "db" in sig.parameters

    def test_unified_endpoint_passes_db(self):
        """unified endpoint 调用 generate_all_parallel 时传了 db session。"""
        import inspect
