...
class TestLLMClient:
    ...
    async def test_call_returns_parsed_json(self):
        ...
        mock_response.output_text = '{"goal": "test goal", "composition": "centered"}'

        client = LLMClient()
        with patch.object(client._client.responses, "create", new_callable=AsyncMock, return_value=mock_response):
